...
/**
 * Modifica el stepTime por defecto para que solo se muestre la capa que 
 * corresponde al tiempo exacto.
 */
L.TimeDimension.Layer.TimeGeoJson = L.TimeDimension.Layer.GeoJson.extend({
    ...
    _getFeatureBetweenDates: function (feature, minTime, maxTime) {
        ...
        if (feature.geometry.coordinates[0].length) {
            new_coordinates = feature.geometry.coordinates.slice(0, featureTimes.length);
        } else {
    ...
    maxTime: function () {
        return Math.max(...this._availableTimes);
    },
    ...
    update: function(data) {
        for (var i = 0; i < data.length; i++) {
            ...
            for (var j = 0; j < update.length; j++) {
                this._baseLayer.addData(update[j]);
            }
        }
    }
...
L.timeDimension.layer.timeGeoJson = function (layer, options) {
    return new L.TimeDimension.Layer.TimeGeoJson(layer, options);
};

L.TimeDimension.Layer.TileLayer.MeteosatelXYZ = L.TimeDimension.Layer.TileLayer.Portus.extend({
    
    initialize: function (layer, options) {
        L.TimeDimension.Layer.TileLayer.prototype.initialize.call(this, layer, options);
        this._layers = {};
        this._defaultTime = 0;
        this._availableTimes = options.times || [];
        this._validFor = options.validFor || 5;
        
        this._timeCacheBackward = this.options.cacheBackward || this.options.cache || 3;
        this._timeCacheForward = this.options.cacheForward || this.options.cache || 3;
    ...
    _getNearestTime: function(time) {
        ...
        if (time != this._availableTimes[index]) {
            /** dif Diferencia en minutos respecto al tiempo esperado */
            var dif = (time - this._availableTimes[index]) / 60000;
            if (Math.abs(dif) > this._validFor + 1) {
                //La diferencia no puede ser mayor que la vigencia de la capa.
                return time;
            }
        }
        return this._availableTimes[index];
...
L.timeDimension.layer.tileLayer.meteosatelXYZ = function (layer, options) {
    return new L.TimeDimension.Layer.TileLayer.MeteosatelXYZ(layer, options);
};